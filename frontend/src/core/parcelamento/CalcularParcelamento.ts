import { QTED_MAX_PARCELAS, TAXA_JUROS_MENSAL } from "@/core/index";
import Parcelamento from "./Parcelamento";

export default class CalcularParcelamento {
  executar(
    valor: number,
    qtdeParcelas: number = QTED_MAX_PARCELAS,
    taxaJuros: number = TAXA_JUROS_MENSAL
  ): Parcelamento {
    if (qtdeParcelas < 2 || qtdeParcelas > QTED_MAX_PARCELAS) {
      throw new Error(
        `Quantidade de parcelas deve ser entre 2 e ${QTED_MAX_PARCELAS}`
      );
    }
